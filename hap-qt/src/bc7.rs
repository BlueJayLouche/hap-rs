...
//! palette entry. If pixel 0 lands on an index >= 8 the endpoints are swapped
//! (indices map `i -> 15 - i` exactly, since the 4-bit weight table satisfies
//! `w[15 - i] == 64 - w[i]`), because the anchor index is stored in 3 bits.
...
pub fn compress_bc7_mode6(rgba: &[u8], width: usize, height: usize, out: &mut [u8]) {
